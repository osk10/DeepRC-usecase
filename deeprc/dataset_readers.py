...
def make_dataloaders(task_definition: TaskDefinition, metadata_file: str, repertoiresdata_path: str,
                     ...
                     sequence_counts_scaling_fn: Callable = no_sequence_count_scaling, verbose: bool = True, one_loader: bool = False) \
    ...
    if one_loader:
        dataloader = _make_dataloader(full_dataset, shuffled_repertoire_inds)
        return dataloader


    trainingset_dataloader = _make_dataloader(full_dataset, trainingset_inds, sample_n_sequences, batch_size, True,
                                              n_worker_processes)
    trainingset_eval_dataloader = _make_dataloader(full_dataset, trainingset_inds)
    validationset_eval_dataloader = _make_dataloader(full_dataset, validationset_inds)
    testset_eval_dataloader = _make_dataloader(full_dataset, testset_inds)
...
def _make_dataloader(full_dataset, set_inds, sample_n_sequences=None, batch_size=1, shuffle=False,
                     n_worker_processes=1):
    dataset = RepertoireDatasetSubset(
        dataset=full_dataset, indices=set_inds, sample_n_sequences=sample_n_sequences)

    dataloader = DataLoader(
        dataset, batch_size=batch_size, shuffle=shuffle, num_workers=n_worker_processes, collate_fn=no_stack_collate_fn)
    return dataloader
